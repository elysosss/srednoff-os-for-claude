...
## G2 (Дополнение G — Yandex Direct / Meta+Google Ads / Telegram, добавлено по запросу Ивана 27.07.2026)
2031. `yandex-direct-ppc` [marketing][ppc] GH:Silverov/yandex-direct-skill — Full-cycle Yandex Direct (API v5) management: account audits (50+ checks, A-F scoring), campaign/keyword CRUD, reporting, budget analysis, negative-keyword management. MIT, 43★, verified 27.07.2026.
2032. `paid-ads-operating-contract` [marketing][ppc] GH:AgriciDaniel/claude-ads — Shared operating contract and thinking framework for multi-platform paid-ads audits (scoring, benchmarks, budget allocation, conversion tracking, bidding strategies, compliance). Read this before google-ads-full-audit or meta-ads-full-audit. MIT, 7554★, verified 27.07.2026.
2033. `google-ads-full-audit` [marketing][ppc] GH:AgriciDaniel/claude-ads — Google Ads audit: measurement, Search, Shopping, Performance Max, Demand Gen, YouTube-linked inventory, keywords/search terms, negative-keyword review, creative assets, bidding, budgets, policy. Depends on paid-ads-operating-contract. MIT, 7554★, verified 27.07.2026.
2034. `meta-ads-full-audit` [marketing][ppc] GH:AgriciDaniel/claude-ads — Meta Ads (Facebook/Instagram) audit: measurement, campaign structure, creative, Advantage+/AI stack, bidding, budgets, policy. Depends on paid-ads-operating-contract. MIT, 7554★, verified 27.07.2026.
2035. `telegram-bot-builder` [telegram][backend] GH:davila7/claude-code-templates — Production Telegram bot patterns for Node.js and Python (Bot API v9.4): auth, webhooks vs polling, inline keyboards, callback queries, media, payments/Stars, inline mode, deployment. MIT, 29942★, verified 27.07.2026.
2036. `telegram-ads-channel-placement` [telegram][marketing][ppc] SREDNOFF — Plan/audit campaigns on the official Telegram Ads platform (ads.telegram.org): channel/category targeting, 160-char sponsored-message copy, TON payment, guidelines compliance. No third-party skill exists for this platform (verified 27.07.2026) - written directly from Telegram's own public ad platform docs.
2037. `ai-ads-claude` [marketing][ppc] GH:zubair-trabzada/ai-ads-claude — AI ad-strategy engine: platform-specific copy generation (Google, Meta, LinkedIn, TikTok, YouTube, Pinterest), funnel design, budget allocation, PDF reports. Discovery-tier (not imported to skills-library) - review before use. MIT, 208★, verified 27.07.2026.
2038. `telegram-miniapp-nextjs-template` [telegram][web][frontend] GH:Telegram-Mini-Apps/nextjs-template — Official Telegram Mini Apps starter: Next.js + TypeScript + TON Connect + @telegram-apps/sdk (tma.js). Project-starter template, not an instructional skill (same pattern as vibe-template-web). Official Telegram-Mini-Apps org, 370★; no LICENSE file detected - verify terms before commercial use. Verified 27.07.2026.

## G2 (Дополнение H — Python / Infra / БД: инженерные скиллы для разработчика)

2039. `python-typing-and-mypy-strictness` [backend] SREDNOFF — Use this skill for Python type hints, generics, Protocol, TypedDict, overloads, and strict mypy or pyright configuration, including gradual typing of untyped legacy modules. Trigger when the task involves programming work related to Python Typing And Mypy Strictness, implementation, audits, debugging, strategy, or validation.
2040. `python-async-await-patterns` [backend][perf] SREDNOFF — Use this skill for asyncio design and repair - TaskGroup, structured concurrency, cancellation and timeouts, blocking calls inside the event loop, and the deadlocks they cause. Trigger when the task involves programming work related to Python Async Await Patterns, implementation, audits, debugging, strategy, or validation.
2041. `python-packaging-pyproject-poetry-uv` [backend] SREDNOFF — Use this skill for Python dependency management and release: pyproject.toml metadata, uv or Poetry workflows, lock files, reproducible environments, version constraints, and publishing to an index with trusted publishing. Trigger when the task involves programming work related to Python Packaging Pyproject Poetry Uv, implementation, audits, debugging, strategy, or validation.
2042. `python-dataclasses-pydantic-validation` [backend][data] SREDNOFF — Use this skill for Python data modeling and input validation - Pydantic v2 models, validators and settings, dataclasses versus attrs, request and config schema design, serialization, and clear validation errors. Trigger when the task involves programming work related to Python Dataclasses Pydantic Validation, implementation, audits, debugging, strategy, or validation.
2043. `pytest-fixtures-and-parametrization` [test] SREDNOFF — Use this skill for pytest structure and mechanics - fixture scope and teardown, conftest layering, parametrize and ids, markers and selection, monkeypatch, and plugin choices for slow or flaky suites. Trigger when the task involves programming work related to Pytest Fixtures And Parametrization, implementation, audits, debugging, strategy, or validation.
2044. `python-profiling-cprofile-pyspy` [perf][backend] SREDNOFF — Use this skill to profile Python CPU and wall-clock time with cProfile, py-spy, and flame graphs - choosing the right profiler, sampling a live production process, and reading the output before optimizing anything. Trigger when the task involves programming work related to Python Profiling Cprofile Pyspy, implementation, audits, debugging, strategy, or validation.
2045. `memory-leak-detection-python` [perf][backend] SREDNOFF — Use this skill when a Python process grows in RSS without bound - tracemalloc snapshots, objgraph reference chains, gc diagnostics, reference cycles, caches that never evict, and native allocations that never return to the OS. Trigger when the task involves programming work related to Memory Leak Detection Python, implementation, audits, debugging, strategy, or validation.
2046. `legacy-python-codebase-modernization` [backend][quality] SREDNOFF — Use this skill to modernize an aging Python codebase in stages - raising the minimum interpreter version, replacing abandoned dependencies, introducing typing and linting on a ratchet, and retiring dead code with evidence. Trigger when the task involves programming work related to Legacy Python Codebase Modernization, implementation, audits, debugging, strategy, or validation.
2047. `python-debugger-pdb-production-debugging` [backend][quality] SREDNOFF — Use this skill for hands-on Python debugging - pdb and ipdb sessions, breakpoint and post-mortem workflows, faulthandler and signal dumps, attaching to a stuck or remote process, and reconstructing a production incident from a traceback. Trigger when the task involves programming work related to Python Debugger Pdb Production Debugging, implementation, audits, debugging, strategy, or validation.
2048. `structured-logging-python` [backend][ops] SREDNOFF — Use this skill for Python logging configuration and structured JSON output - stdlib logging versus structlog, contextvars for request correlation, log levels that mean something, exception logging, and rules for what must never be logged. Trigger when the task involves programming work related to Structured Logging Python, implementation, audits, debugging, strategy, or validation.
2049. `sqlalchemy-orm-patterns` [backend][data] SREDNOFF — Use this skill for SQLAlchemy 2.0 ORM work: session lifecycle, unit of work boundaries, relationship loading strategies, and N+1 query elimination. Trigger when the task involves declarative models, repository or service layers, slow ORM-backed endpoints, detached instance errors, or a move from legacy Query API to 2.0 select() style.
2050. `alembic-migration-workflows` [data] SREDNOFF — Use this skill for Alembic migration workflows: autogenerate review, revision branching and merges, downgrade paths, data migrations, and zero-downtime rollout ordering. Trigger when a revision must be authored or reviewed, autogenerate produced a suspicious diff, heads have diverged after a merge, or a schema change must ship without taking the application offline.
2051. `dockerfile-best-practices-python` [infra][backend] SREDNOFF — Use this skill for authoring and reviewing Python Dockerfiles: multi-stage builds, layer ordering and cache hits, non-root runtime users, image size reduction, and .dockerignore hygiene. Trigger when a Python service needs a container image, an existing image is oversized or rebuilds slowly, or a review flags a container running as root.
2052. `docker-compose-local-dev-environments` [infra] SREDNOFF — Use this skill for Docker Compose local development environments: service healthchecks, dependency ordering, volumes for code and data, environment variables driven by a checked-in .env.example, and optional profiles. Trigger when a repository needs a one-command local stack, when services start before their dependencies are ready, or when onboarding a new developer breaks on missing configuration.
2053. `kubernetes-deployment-manifests-basics` [infra] SREDNOFF — Use this skill for writing and reviewing core Kubernetes manifests: Deployment, Service, ConfigMap and Secret, readiness and liveness probes, resource requests and limits, and rollout plus rollback control. Trigger when a service needs its first manifests, when pods restart or never become ready, when a rollout stalls, or when a release must be rolled back.
2054. `github-actions-python-ci-pipeline` [devops][cicd] SREDNOFF — Use this skill for GitHub Actions CI on Python projects: a version matrix, dependency caching, lint, typecheck and test as separate steps, and artifact upload. Trigger when a Python repository needs a CI workflow, when the pipeline is slow because dependencies reinstall every run, when failures are hard to attribute to a stage, or when coverage and build artifacts must be published from CI.

## G3 (Дополнение I — discovery-верификация внешних ссылок, 13.09.2026)

2055. `archify` [arch][docs][diagram] GH:tt-a1i/archify — Deterministic architecture-diagram compiler: the agent emits typed JSON IR, `archify` compiles it into a self-contained interactive HTML/SVG (architecture, workflow, sequence, dataflow, lifecycle) with a machine validation gate (`node bin/archify.mjs validate --quality showcase`), PNG/SVG/WebM export, Mermaid import, and Before/Delta/After snapshot diff. Discovery-tier (not imported to skills-library - SKILL.md is inseparable from its Node runtime, `schemas/` and `examples/`); install with `npx skills add tt-a1i/archify -g`, requires Node. MIT, 60408★, verified 13.09.2026.
2056. `headroom` [infra][perf][mcp] GH:headroomlabs-ai/headroom — Local context-compression layer for LLM tool output (library + HTTP proxy + MCP server + `headroom wrap claude`): content-type routing to SmartCrusher (JSON), CodeCompressor (AST) and Kompress (prose), KV-cache alignment, and local originals re-fetchable via `headroom_retrieve`. External tool, not a skill; sits on the path of all LLM traffic, so pin a tag and review before use, and do not let `headroom learn` write to `CLAUDE.md` or `AGENTS.md` (restrict it to gitignored `CLAUDE.local.md`). Apache-2.0, 71843★, verified 13.09.2026.
2057. `strix` [security][test] GH:usestrix/strix — Autonomous penetration-testing agent that runs sandboxed exploitation in containers plus 9 bundled security skills (OWASP Top 10, web app, API, CI scanning, find/fix vulnerabilities). Discovery-tier (not imported to skills-library) - autonomous framework that makes edits and runs exploitation, so per `70-skills-registry.md` (swarm/autonomous frameworks, 50-security.md) install and run ONLY with explicit confirmation from the owner; for routine work prefer the existing canon `security-auditor` or `penetration-tester`. Apache-2.0 (cannot be relicensed into this MIT tree), 62199★, verified 13.09.2026.
2058. `open-notebook` [research][ai][data] GH:lfnovo/open-notebook — Self-hosted open-source NotebookLM alternative: private research workspace over your own sources with multi-model support, notes, search and podcast generation. Standalone application, no SKILL.md - discovery-tier; complements `PleasePrompto:notebooklm-skill`, which drives Google's hosted NotebookLM instead. MIT, 38691★, verified 13.09.2026.
2059. `open-seo` [seo][mcp][data] GH:every-app/open-seo — Open-source Semrush/Ahrefs alternative with an MCP server over DataForSEO: keyword research, rank tracking, backlink and competitor data exposed to the agent as a live data source rather than as advice. Discovery-tier data source (not imported to skills-library - its bundled skills are wrappers over its own MCP and are useless without the deployed service); needs your own DataForSEO key. MIT, 18588★, verified 13.09.2026.
2060. `claude-video` [ai][research] GH:bradautomates/claude-video — `/watch <url|path>` pipeline that lets the agent actually see a video: yt-dlp download, ffmpeg scene-aware frame extraction, native subtitles with a Whisper API fallback, then frames read as images alongside the transcript. Discovery-tier (not imported to skills-library - requires the yt-dlp and ffmpeg system binaries plus a paid transcription fallback); last push 01.07.2026. MIT, 17153★, verified 13.09.2026.
2061. `i-have-adhd` [meta][docs] GH:ayghri/i-have-adhd — Explicitly invoked output-style skill (`/i-have-adhd`, `disable-model-invocation: true`): action first, numbered steps, restated state, trimmed tangents, concrete time estimates, visible progress. Discovery-tier, personal preference rather than a capability - it conflicts with the mandatory final-answer format in `CLAUDE.md` §6, so enable it deliberately and only per session. MIT, 43978★, verified 13.09.2026.
2062. `book-to-skill` [meta][docs][research] GH:virgiliojr94/book-to-skill — Turns a technical book (PDF/EPUB) into a Claude Code skill: chunked extraction, structure detection, skill scaffolding. Discovery-tier, NOT imported to skills-library - its SKILL.md is 46 KB and hard-depends on `scripts/extract.py` plus a Python package, so a single-page copy would fail on its first step; install from the repo instead. Note upstream ships a SECURITY-NOTICE.md about a malicious re-upload under a similar name - pin this owner and a commit. MIT, 30400*, verified 13.09.2026.
2063. `no-ai-slop` [docs][quality] GH:petergyang/no-ai-slop — Edit prose out of generic AI phrasing while keeping the author's voice, or audit a draft without rewriting it. Adapted into skills-library (MIT, attribution in INSTALL-SOURCES.md); supersedes the text-only `unslop` entry as the canon for prose de-slopping. MIT, 9035*, verified 13.09.2026.
