...
| Skill library (curated, 325) | `templates/claude-md-os/skills-library/*/SKILL.md` | No - `gen-profile-lock` installs a capped, tag-matched subset into a project's `.claude/skills/` |
...
pattern manually. A curated subset (325 records - 303 adapted from `SREDNOFF`, 6 later imports from external
MIT sources, and 16 Python/infra skills written for this repo, see
...
start, roughly 100 tokens each even when unused - installing all 325 unconditionally
