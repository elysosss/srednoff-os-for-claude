...
| Skill library (curated, 326) | `templates/claude-md-os/skills-library/*/SKILL.md` | No - `gen-profile-lock` installs a capped, tag-matched subset into a project's `.claude/skills/` |
...
pattern manually. A curated subset (326 records - 303 adapted from `SREDNOFF`, 7 later imports from external
...
start, roughly 100 tokens each even when unused - installing all 326 unconditionally
