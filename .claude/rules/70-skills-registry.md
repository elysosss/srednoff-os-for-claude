...
текстовый каталог (грепай, читай, применяй вручную). Отдельная кураторская подгруппа (325 записей:
303 адаптированы из `SREDNOFF`, 6 более поздних импортов и 16 собственных Python/infra-скиллов,
см. `INSTALL-SOURCES.md`) имеет реальный `SKILL.md`-контент в
...
## Базовые скиллы проекта

В `.claude/skills` лежат пять статических скиллов: `github-research`, `product-builder`,
`production-review`, `connector-orchestrator`, `project-bootstrap` — вызывай их, когда задача
подходит. Скиллы существуют для процедур, слишком длинных, чтобы постоянно висеть в загружаемом
`CLAUDE.md`.
