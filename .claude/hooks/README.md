...
- `require-profile-lock-read.ps1` / `.sh` — PreToolUse(Edit|Write|MultiEdit): **не безопасность, а compliance-гейт**. Денит первый Edit/Write/MultiEdit за сессию в проекте, где есть `.claude/PROFILE.lock.md`, пока он реально не прочитан (по маркеру от `mark-profile-lock-read`). Один раз за сессию — дальше не мешает. Fail-open по конструкции (баг здесь не должен блокировать работу, в отличие от security-хуков выше). Причина существования: реальный кейс, где баннер `SREDNOFF OS — ACTIVE` в начале `CLAUDE.md` не заставил агента прочитать lock-файл — пассивный текст в контексте не равен исполнению (см. `70-skills-registry.md`).
